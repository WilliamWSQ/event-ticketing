...
const Bar = styled.nav<{ $scrolled: boolean }>`
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
...
`;
const Account = styled.button<{ $active: boolean }>`
  display: flex;
  align-items: center;
  gap: 9px;
  height: 40px;
  ...
  return (
    <Bar $scrolled={scrolled}>
      ...
      <Inner>
        ...
        <Links>
          <Link $active={!onAccount} onClick={() => navigate('/')}>
            {t.navAfisha}
            {!onAccount && <Underline aria-hidden="true" />}
          </Link>
          <Link onClick={() => navigate('/')}>{t.navFest}</Link>
          <Link onClick={() => navigate('/')}>{t.navVenues}</Link>
        </Links>
        ...
        <IconBtn onClick={() => navigate('/ticket')} title={t.myTickets} aria-label={t.myTickets}>
          <Ticket size={18} strokeWidth={1.7} />
          <Dot aria-hidden="true" />
        </IconBtn>

        <Account $active={onAccount} onClick={() => navigate('/account')}>
          <Avatar>{t.initials}</Avatar>
          <UserName>{t.userShort}</UserName>
        </Account>
      </Inner>
    </Bar>
  );
